...
public class ShadowSocksDetailsEntity implements Serializable {
	...
	public String getLinkNotSafe() {
		...
		link
				...
				.append(SSR_LINK_SEPARATOR).append(method)
				.append(SSR_LINK_SEPARATOR).append(obfs)
				.append(SSR_LINK_SEPARATOR).append(Base64.encodeBase64String(password.getBytes()))
				.append("/?obfsparam=")
				.append("&protoparam=")
				.append("&remarks=").append(Base64.encodeBase64String(remarks.getBytes()))
				.append("&group=").append(Base64.encodeBase64String(group.getBytes()));
